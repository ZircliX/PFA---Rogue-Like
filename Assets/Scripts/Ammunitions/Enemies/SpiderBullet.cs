...
using DeadLink.Ammunitions.Data.Enemies;
...
{
    public class SpiderWeapon : Bullet
    {
        public override BulletData BulletData => bulletData;
        [SerializeField] private SpiderBulletData bulletData;
            ...
            if (distance > 0)
            {
                ...
                if (Physics.Raycast(ray, out RaycastHit hit, distance))
                {
                    if (hit.collider.gameObject.GetInstanceID() != gameObject.GetInstanceID())
                    {
                        ...
                        RaycastHit[] sphereCastAll = Physics.SphereCastAll(hit.point, bulletData.DamageRadius, hit.transform.forward) ?? throw new ArgumentNullException("Physics.SphereCastAll(hit.point, RocketBulletData.ExplosionRadius, hit.transform.forward)");
