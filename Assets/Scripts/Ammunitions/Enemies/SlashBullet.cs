...
namespace DeadLink.Ammunitions.Enemies
{
    public class SlashBullet : Bullet
    {
        ...
        [SerializeField] private SlashBulletData bulletData;
            ...
            if (distance > 0)
            {
                ...
                if (Physics.Raycast(ray, out RaycastHit hit, distance))
                {
                    if (hit.collider.gameObject.GetInstanceID() != gameObject.GetInstanceID())
                    {
                        ...
                        RaycastHit[] sphereCastAll = Physics.SphereCastAll(hit.point, bulletData.DamageRadius, hit.transform.forward) 
                                                     ?? throw new ArgumentNullException("Physics.SphereCastAll(hit.point, RocketBulletData.ExplosionRadius, hit.transform.forward)");
